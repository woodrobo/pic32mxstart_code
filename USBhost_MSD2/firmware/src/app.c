...
USB_ALIGN uint8_t readData[100];
int readSize;
...
void APP_Tasks ( void )
{
    switch(appData.state)
    {
        ...
        case APP_STATE_OPEN_FILE:
            ...
            appData.fileHandle = SYS_FS_FileOpen("/mnt/myDrive1/file.txt", (SYS_FS_FILE_OPEN_READ_PLUS));
            ...
            else
            {
                ...
                appData.state = APP_STATE_READ_AND_CLOSE_FILE;

            }
            break;
            
        case APP_STATE_READ_AND_CLOSE_FILE:
            
            readSize = SYS_FS_FileRead( appData.fileHandle, (void *) readData, 100 );
            ...
            {
                appData.state = APP_STATE_OPEN_FILE2;
            }
            
            break;
            
        case APP_STATE_OPEN_FILE2:

            /* Try opening the file for append */
            appData.fileHandle = SYS_FS_FileOpen("/mnt/myDrive1/copy.txt", (SYS_FS_FILE_OPEN_APPEND_PLUS));
            if(appData.fileHandle == SYS_FS_HANDLE_INVALID)
            {
                /* Could not open the file. Error out*/
                appData.state = APP_STATE_ERROR;
        ...
        case APP_STATE_WRITE_TO_FILE:
            ...
            if (SYS_FS_FileWrite( appData.fileHandle, (const void *) readData, readSize ) == -1)
            {
