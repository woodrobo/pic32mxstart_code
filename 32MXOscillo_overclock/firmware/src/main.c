...
#define MIN_SAMPLING_PERIOD 47  //48MHz TMRで1us
...
void oscillo_callback(UART_EVENT event, uintptr_t context){
    if(!serial_buf.is_complete){
        while(UART1_ReadCountGet()){
            if(UART1_Read(&serial_buf.data[serial_buf.counter], 1)){
                serial_buf.counter++;
                if(serial_buf.data[serial_buf.counter-1] == 0){
                    serial_buf.is_complete = true;
                    break;
                }
                if(serial_buf.counter == 10){
...
int main ( void )
{
    ...
    UART1_ReadCallbackRegister(oscillo_callback, (uintptr_t)NULL);
    ...
    while ( true )
    {
        ...
        while(oscillo_state.mode != OSCILLO_WAIT && !serial_buf.is_complete){
            //AD変換結果を変数に一時保存
            while(!AD1CON1bits.DONE);
            uint16_t adc_result = ADC1BUF0;
